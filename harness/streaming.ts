...
import { analyseArrivalOrder, type IPriorityReport } from './streaming-priority';
...
scene.fog = new THREE.FogExp2(SKY.getHex(), 0.0006);
...
/**
 * Did the city assemble in front of the camera? Binds the cold-start capture to
 * `analyseArrivalOrder`; the maths (and its long rationale) lives in
 * `harness/streaming-priority.ts`, where it is unit-tested.
 */
function priorityReport(): IPriorityReport {
  return analyseArrivalOrder(arrivalOrder, coldStartOrigin, coldStartForward);
}
...
async function setCameraForShot(): Promise<void> {
  ...
  // High over the southern edge looking north across the whole district, so all
  // four bands land in one frame: R0 fracture-piece detail underneath, the R1
  // and R2 merged blocks stepping away, and the baked impostor skyline on the
  // horizon fading into the fog.
  camera.position.set(-330, 155, 470);
  camera.lookAt(190, 12, -540);
  camera.updateMatrixWorld();
  await settle();
